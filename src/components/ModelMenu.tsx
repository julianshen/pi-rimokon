...
export function ModelMenu({ model, onPick }: ModelMenuProps) {
  return (
    ...
    >
      <div style={{ padding: '7px 10px 4px', fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, letterSpacing: '.08em', textTransform: 'uppercase', color: '#9b9788' }}>
        Switch model · ⌃L
      </div>
      {MODELS.map((m) => (
        <button
        ...
        >
          <span style={{ width: 7, height: 7, borderRadius: '50%', flex: 'none', background: m.id === model ? '#1f8a5b' : 'transparent' }} />
          <span style={{ flex: 1, minWidth: 0 }}>
            <span style={{ fontSize: 13, fontWeight: 600, display: 'block' }}>{m.label}</span>
            <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, color: '#9b9788' }}>{m.provider}</span>
          </span>
        </button>
      ))}
    </div>
